...
if [ "$program" == "jmavsim" ] && [ ! -n "$no_sim" ]; then
	...
	if [ -x "$(command -v gazebo)" ]; then
		...
		while gz model --verbose --spawn-file="${modelpath}/${model}/${model_name}.sdf" --model-name=${model} -x 1.01 -y 0.98 -z 0.05 2>&1 | grep -q "An instance of Gazebo is not running."; do
